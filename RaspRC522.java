...
public class RaspRC522 {
  ...
  private byte Read_RC522(byte address) {
    ...
    data[1] = 0;
    int result = Spi.wiringPiSPIDataRW(SPI_Channel, data);
    if (result == -1) System.out.println("Device read  error,address=" + address);
    return data[1];
  }
  ...
  private int Write_Card(
      byte command, byte[] data, int dataLen, byte[] back_data, int[] back_bits, int[] backLen) {
    int status = MI_ERR;
    ...
    for (i = 0; i < dataLen; i++) Write_RC522(FIFODataReg, data[i]);

    Write_RC522(CommandReg, command);
    if (command == PCD_TRANSCEIVE) SetBitMask(BitFramingReg, (byte) 0x80);
    ...
    if (i != 0) {
      if ((Read_RC522(ErrorReg) & 0x1B) == 0x00) {
        status = MI_OK;
        if ((n & irq & 0x01) > 0) status = MI_NOTAGERR;
        if (command == PCD_TRANSCEIVE) {
          n = Read_RC522(FIFOLevelReg);
          lastBits = (byte) (Read_RC522(ControlReg) & 0x07);
          if (lastBits != 0) back_bits[0] = (n - 1) * 8 + lastBits;
          else back_bits[0] = n * 8;

          if (n == 0) n = 1;
          if (n > this.MAX_LEN) n = this.MAX_LEN;
          backLen[0] = n;
          for (i = 0; i < n; i++) back_data[i] = Read_RC522(FIFODataReg);
        }
      } else status = MI_ERR;
    }
    return status;
  ...
  public int AntiColl(byte[] back_data) {
    ...
    if (status == MI_OK) {
      if (backLen[0] == 5) {
        for (i = 0; i < 4; i++) serial_number_check ^= back_data[i];
  ...
  private void Calculate_CRC(byte[] data) {
    ...
    for (i = 0; i < data.length - 2; i++) Write_RC522(FIFODataReg, data[i]);
  ...
  public int Select_Tag(byte[] uid) {
    ...
    data[1] = 0x70;
    for (i = 0, j = 2; i < 5; i++, j++) data[j] = uid[i];
    Calculate_CRC(data);
  ...
  // Authenticates to use specified block address. Tag must be selected using select_tag(uid) before
  // auth.
  ...
  public int Auth_Card(byte auth_mode, byte block_address, byte[] key, byte[] uid) {
    ...
    for (i = 0, j = 2; i < 6; i++, j++) data[j] = key[i];
    for (i = 0, j = 8; i < 4; i++, j++) data[j] = uid[i];
  ...
  public int Write(byte block_address, byte[] data) {
    ...
    // System.out.println("back_bits[0]="+back_bits[0]+",(back_data[0] & 0x0F)="+(back_data[0] &
    // 0x0F));
    if (status != MI_OK || back_bits[0] != 4 || (back_data[0] & 0x0F) != 0x0A) status = MI_ERR;
    if (status == MI_OK) {
      for (i = 0; i < data.length; i++) buff_write[i] = data[i];
      Calculate_CRC(buff_write);
      status =
          Write_Card(PCD_TRANSCEIVE, buff_write, buff_write.length, back_data, back_bits, backLen);
  ...
  public byte[] DumpClassic1K(byte[] key, byte[] uid) {
    ...
    for (i = 0; i < 64; i++) {
      ...
      if (status == MI_OK) {
        ...
        if (status == MI_OK) System.arraycopy(buff, 0, data, i * 64, 16);
      }
    }
    return data;
  }
